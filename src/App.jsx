...
const fetchEpsData = async (code) => {
  try {
    ...
    const res = await fetch(
      `https://api.allorigins.win/get?url=${encodeURIComponent(
        `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${symbol}?modules=defaultKeyStatistics,financialData,earningsTrend`
      )}`
    );
    const data = await res.json();
    const parsed = JSON.parse(data.contents);
...
export default function App() {
  ...
  const handleDemoMode = () => {
    setStockName("トヨタ自動車（デモデータ）");
    setCurrentPrice("3850");
    setCompanyEps("380");
    setShikihoEpsNow("370");
    setShikihoEpsNext("420");
    setMidData({
      finalYear: "2030年3月期",
      operatingProfit: "3000",
      netProfitRatio: "60",
      shares: "134000",
      source: "https://global.toyota/pages/global_toyota/ir/",
      notes: "※デモデータです。実際の数値とは異なります。"
    });
    setMidEps(Math.round((3000 * 100000000 * 0.60 / (134000 * 10000)) * 10) / 10);
    setFetchError("");
    setResults(null);
